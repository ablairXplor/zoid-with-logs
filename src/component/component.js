...
export function component<P, X, C>(opts : ComponentOptionsType<P, X, C>) : Component<P, X, C> {
    ...
    const registerChild = memoize(() : ?ChildComponent<P, X> => {
          console.log("[ZOID] Registering child component:", tag);
        if (isChild()) {
            if (window.xprops) {
    ...
    const init = (inputProps? : PropsInputType<P> | void) : ZoidComponentInstance<P, X, C> => {console.log("[ZOID] Initializing component:", tag);// eslint-disable-next-line prefer-conslet instance;
            console.error("[ZOID] Initializing component:", tag);
        ...
        const render = (target, container, context) => {
              ...
              console.error("[ZOID] Rendering component:", tag);
            return ZalgoPromise.try(() => {
                if (!eligibility) {
...
export function destroyComponents(err? : mixed) : ZalgoPromise<void> {
       console.error("[ZOID] Destroying all components:", err);
    if (bridge) {
...
export function destroy(err? : mixed) : ZalgoPromise<void> {
      console.error("[ZOID] Destroying component:", tag, err);
    destroyAll();
    destroyGlobal();
    destroyPostRobot();
